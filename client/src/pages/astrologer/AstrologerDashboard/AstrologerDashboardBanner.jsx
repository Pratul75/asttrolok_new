...
const AstrologerDashboardBanner = () => {
  ...
  const getUserConsultations = async () => {
    try {
      const res = await axios.get(
        `http://localhost:4000/api/astrologer/getAstrologerAllConsultation`,
        {
          headers: {
      ...
      console.log('AstrologerDashboardBanner.jsx', res);
      ...
      <div className="flex flex-col md:flex-row justify-between h-auto bg-[#EBF3FE] rounded-lg p-6 md:p-[30px] md:w-3/4">
        <div className="flex flex-col gap-3 items-center md:items-start justify-around">
          <div className="flex gap-4 items-center">
            ...
            <h2 className="font-semibold">Welcome back {loginResponse?.name?.toUpperCase() || "USER"}</h2>
          </div>
...
export default AstrologerDashboardBanner;
